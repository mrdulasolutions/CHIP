...
copy_minimal() {
  local dest="$1"
  mkdir -p "$dest/scripts" "$dest/models" "$dest/rag/index" "$dest/tmp"
  rsync -a "${RSYNC_EXCLUDES[@]}" "$ROOT/scripts/" "$dest/scripts/"
  rsync -a "${RSYNC_EXCLUDES[@]}" \
    "$ROOT/download-runtime.sh" \
    "$ROOT/download-embed-model.sh" \
    "$ROOT/download-chip-model.sh" \
    "$ROOT/download-pdf-tools.sh" \
  ...
  for f in "Launch CHIP.command" "Launch CHIP.bat" "Launch CHIP.sh"; do
...
chmod_kit() {
  ...
  chmod +x "$dest/Launch CHIP.sh" 2>/dev/null || true
  chmod +x "$dest/Launch CHIP.command" 2>/dev/null || true
}
...
Grid-down on a new computer:
  • macOS:   double-click "Launch CHIP.command" (or Terminal: ./start-rag.sh or ./start.sh chip)
...
See README.md and docs/AUTOSTART.md for shortcuts and troubleshooting.
EOF
